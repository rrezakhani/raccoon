...
#include "ComputePorosity.h"
#include "RankTwoTensor.h"

registerMooseObject("raccoonApp", ComputePorosity);

InputParameters
ComputePorosity::validParams()
{
  InputParameters params = Material::validParams();
  params.addClassDescription("compute porosity in poroelastic media");
...
ComputePorosity::ComputePorosity(const InputParameters & parameters)
  : Material(parameters),
    _porepressure(coupledValue("porepressure")),
    _strain(getMaterialPropertyByName<RankTwoTensor>("total_strain")),
...
void
ComputePorosity::initQpStatefulProperties()
{
  _porosity[_qp] = _initial_porosity;
}

void
ComputePorosity::computeQpProperties()
{
  _porosity[_qp] = _porepressure[_qp]/_M[_qp] + _alpha[_qp] * (_strain[_qp].trace()) + _initial_porosity;
}
